...
class SingleBatCreator {
  constructor() {
    this.projectRoot = path.resolve(__dirname, '..');
    this.distBetaPath = path.join(this.projectRoot, 'dist-beta');
  }

  async createSingleBat() {
    console.log('🔧 Creating single comprehensive batch file...');
    ...
    try {
      ...
      await this.createLauncherBatchFile();
      ...
      console.log('\n✅ Single batch file created successfully!');
      
    } catch (error) {
      console.error('❌ Failed to create single batch file:', error.message);
      throw error;
    }
  }
...
`;
    ...
    console.log('   ✅ Single batch file created:', batchPath);
  }

  async createLauncherBatchFile() {
...
`;
    ...
    const launcherPath = path.join(this.distBetaPath, 'launch-app.bat');
    await fs.writeFile(launcherPath, launcherContent);
    console.log('   ✅ Launcher batch file created:', launcherPath);
  }

  async createUserReadme() {
    console.log('📝 Creating user README...');
    
    const readmeContent = `# NW Buddy Scraper - All-in-One Launcher
...
**NW Buddy Scraper - All-in-One Launcher**
...
3. Run the new \`START-NW-BUDDY-SCRAPER.bat\`
...
`;
  ...
  async testBatchFile() {
    console.log('🧪 Testing single batch file...');
    ...
    if (!fs.existsSync(batchPath)) {
      throw new Error('Single batch file not found');
    }
    
    if (!fs.existsSync(launcherPath)) {
      throw new Error('Launcher batch file not found');
    }
    ...
    console.log('   ✅ Single batch file found:', batchPath);
    console.log('   ✅ Launcher batch file found:', launcherPath);
    console.log('   ✅ README file found:', readmePath);
    console.log('   ✅ Single batch file looks good');
  }
}

// Run the creator if this script is executed directly
if (require.main === module) {
  const creator = new SingleBatCreator();
  creator.createSingleBat().then(() => {
    console.log('\n🎉 Single batch file creation completed!');
    console.log('\n📋 Summary:');
    console.log('✅ Created single comprehensive batch file');
    console.log('✅ Created user README');
    console.log('✅ Tested batch file');
    console.log('\n🚀 Your single batch file is ready!');
    ...
    console.error('\n💥 Single batch file creation failed!');
    process.exit(1);
  });
}

module.exports = SingleBatCreator; 